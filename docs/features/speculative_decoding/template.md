...
A draft is proposed while the generated output is a prefix of one of its active templates. Exact token prefixes use a trie. When a tokenizer represents an intermediate character prefix differently from the complete template, vLLM falls back to character-prefix matching and proposes the remainder only if contextual detokenization verifies that it reconstructs the same template text. Ambiguous or unsupported reconstructions receive no proposal.

The service-level templates are the default, but each request can inherit, disable, or override them. When several templates share a common prefix (e.g. verdicts differing only in a `yes`/`no` token), the earliest template in the list is proposed; if the decision token is rejected, the token emitted by rejection sampling reveals the actual branch and the matching template takes over on the next step.
...
!!! tip "Choosing `num_speculative_tokens`"
    Proposals are capped at the matched template's remaining tokenization, so set `num_speculative_tokens` to at least the token length of your longest template (plus one for the EOS token appended by default) to verify a full response in a single step. Alternate valid tokenizations can require additional tokens and may complete over more than one step.
